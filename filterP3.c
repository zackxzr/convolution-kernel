...
int getPixel(int px, int py, int l, PPMImage *img)
{
    switch (l)
    {
    // Red
...
/*Zero padding*/
int getPixel_check(int px, int py, int l, PPMImage *img)
{
...
PPMImage *readPPM(char *filename)
{
    ...
    {
        printf(" '%s' doesn't have 8-bits component \n", filename);
        return NULL;
    }
    ...
    while(index < img->width * img->height){
        fscanf(fp, "%d %d %d", &(img->pixel[index]).r, &(img->pixel[index]).g, &(img->pixel[index]).b);
        index++;
    }
    fclose(fp);
    return img;
...
void writePPM(PPMImage *img, char *filename)
{
    ...
    int index = 0;
    while(index < img->width * img->height){
        fprintf(fp, "%d %d %d ", (img->pixel[index]).r, (img->pixel[index]).g, (img->pixel[index]).b);
        index++;
        if(index%7 == 0){
            fprintf(fp,"\n");
        }
...
PPMImage *applyKernel(PPMImage *img, char *kernelfile)
{
    ...
    FILE *fp = fopen(kernelfile, "r");
    char buf[16];
    if (!fp){
        printf("Unable to open file %s", kernelfile);
        return NULL;
    }
    int Ks;
    int div;
    ...
    for (int ix = 0; ix < img->width; ix++)
    {
        for (int iy = 0; iy < img->height; iy++)
        {
            ...
            /*Assigning to image*/
            (out_image->pixel[ix + iy * out_image->width]).r = (int)cp[0];
            (out_image->pixel[ix + iy * out_image->width]).g = (int)cp[1];
            (out_image->pixel[ix + iy * out_image->width]).b = (int)cp[2];
        }
    }
    free(K);
    return out_image;
...
int main(int argc, char **argv){
    ...
    img = readPPM(input);
    out_img = applyKernel(img, kernel);
    writePPM(out_img, output);
